function [appData, handles, ERRORFLG] = Confocal2ColorALExSolnMultDataPhotonRecoloring7(...
    appData, handles)
%
...
%       KGryte - (2012-08-09) - decorrelating E/S
%
%   References:
...
WINDOW = 5;%10; %appData.BVA.Config.Algorithm.Window; % e.g., 5 photons
CORRWINDOW = 5;%10;
sigAlpha = 0.05;
filterFLG = false;
...
%             fprintf('%d Rex photon(s) in wrong bin. Burst No: %d\n', sum(mod(RexIDX,2) == 0), BURST);
    ...
    % [5] Permutation test for significance: 
    numPerms = 100; % number of repeats
    IDX = 1;
    ...
    if filterFLG
        ...
        while permFLG
            ...
            % Find the windows where the correlation is sufficiently
            % significant:
            sigIDS = find(pValues < sigAlpha);
            ...
            % remove the photons within the correlation window:
            % 
            for id = 1 : numel(sigIDS)

                % Original indexes:
                idx = [1:1:WINDOW] + (sigIDS(id)-1)*WINDOW;

                % mix up the indexes:
    %             mixedIdx = randperm(WINDOW) + (sigIDS(id)-1)*WINDOW; % shifts the IDX to the appropriate window

                % Shuffle the photons:
                temp(idx,:) = []; %temp(mixedIdx,:);

            end % end FOR id

            % Check!!! Need to ensure we have not removed all photons...
            if size(temp(:,1),1) < WINDOW 
                EfficiencyVector = nan;
                StoichiometryVector = nan;
                break;
            end % end IF
            ...
            numWindows = floor(size(temp(:,1),1) / WINDOW);% floor(sum((recoloredPhotons(:,2)==0) + (recoloredPhotons(:,2)==2)) / WINDOW); % number of recolored Gex photons
            EfficiencyVector = nan(numWindows, 1);
            StoichiometryVector = nan(numWindows, 1);

            filteredPhotons = temp;
            for win = 1 : numWindows % non-overlapping windows 

                % Get the photon streams in the window:
                DexDem = sum(filteredPhotons(1:WINDOW,2) == 0);
                DexAem = sum(filteredPhotons(1:WINDOW,2) == 2);
                AexDem = sum(filteredPhotons(1:WINDOW,2) == 3);
                AexAem = sum(filteredPhotons(1:WINDOW,2) == 1);
                ...
                % Remove the photons from the stream:
                filteredPhotons(1:WINDOW,:) = []; % recoloredPhotons(1:IDS(end),:) = []; % makes the array smaller at each iteration, thus allowing us to index from 1 as done above.
...
rates = '(1000,10000)';
svgLine = 'acceptorblinking_unfiltered_k1000_k10000';
