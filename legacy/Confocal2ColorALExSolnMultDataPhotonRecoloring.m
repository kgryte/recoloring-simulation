function [appData, handles, ERRORFLG] = Confocal2ColorALExSolnMultDataPhotonRecoloring(...
    appData, handles)
%
...
stoichiometryStateMeans = [...
    0.50;... % S = 0 --> Donor blinking ; S = 1 --> Acceptor blinking
    0.70];
...
ANIMATION.handle = AnimationProgressBar('initialize',...
    ...
    'process', {...
        ...
        'BVA',...
        'Cluster Statistics'},...
    'title', {'Burst Variance Analysis (recoloring)'},...
    'color', {[0 0 1; 0 1 0; 1 0 0]},...
    'Pointer', 'hourglass',...
    'info', sprintf('Beginning Burst Variance Analysis (recoloring)...'));
...
% For Burst Variance Analysis, we need to retain all photons, both those
...
%         between laser excitations.
    ...
    % [4] Slide a window W and calculate S:
    numWindows = floor(size(recoloredPhotons,1) / WINDOW); 
    StoichiometryVector = nan(numWindows, 1);
    
    for win = 1 : numWindows
        
        % Get the photon streams in the window:
        DexDem = sum(recoloredPhotons(1:WINDOW,2) == 0);
        DexAem = sum(recoloredPhotons(1:WINDOW,2) == 2);
        AexDem = sum(recoloredPhotons(1:WINDOW,2) == 3);
        AexAem = sum(recoloredPhotons(1:WINDOW,2) == 1);
        
        % Calculate S:
        StoichiometryVector(win) = (DexDem + DexAem) ./ (DexDem + DexAem + AexAem + AexDem);
        
        % Remove the photons from the stream:
        recoloredPhotons(1:WINDOW,:) = []; % makes the array smaller at each iteration, thus allowing us to index from 1 as done above.
...
myIDS = find(~isnan(appData.BVA.Data.Results(:,6)) & appData.BVA.Data.Results(:,6) < 0.05);
bca = appData.BVA.Data.Results;
...
figure; 
plot(appData.BVA.Data.Results(:,5), appData.BVA.Data.Results(:,6), 'b*')
...
% figure; 
% plot(...
%     appData.BVA.Data.Results(appData.BVA.Data.Results(:,7) > 14 & appData.BVA.Data.Results(:,7) < 31, 5),...
%     appData.BVA.Data.Results(appData.BVA.Data.Results(:,7) > 14 & appData.BVA.Data.Results(:,7) < 31, 6), 'b*')
% title('Correlation and Significance');
% xlabel('Correlation');
% ylabel('P Value');
...
figure; 
plot(appData.BVA.Data.Results(:,6), appData.BVA.Data.Results(:,7), 'b*')
...
figure; 
plot(appData.BVA.Data.Results(:,5), appData.BVA.Data.Results(:,7), 'b*',...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.15, 5), appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.15,7), 'g.',...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.05, 5), appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.05,7), 'ro')
...
figure; 
plot(...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) > 0.1,1),...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) > 0.1,2), 'k.',...
    'markersize', 15)
E = 0:0.01:1; SN = sqrt(E.*(1-E)/7); hold on; plot(E, SN, 'r'); hold off;
title('E-BVA: uncorrelated');


figure; 
plot(...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.05,1),...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.05,2), 'k.',...
    'markersize', 15)
E = 0:0.01:1; SN = sqrt(E.*(1-E)/7); hold on; plot(E, SN, 'r'); hold off;
title('E-BVA: correlated');

figure; 
plot(...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.005,1),...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.005,2), 'k.',...
    'markersize', 15)
E = 0:0.01:1; SN = sqrt(E.*(1-E)/7); hold on; plot(E, SN, 'r'); hold off;
title('E-BVA: highly correlated');


figure; 
plot(...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) > 0.1,3),...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) > 0.1,4), 'k.',...
    'markersize', 15)
S = 0:0.01:1; SN = sqrt(S.*(1-S)/10); hold on; plot(S, SN, 'r'); hold off;
title('S-BVA: uncorrelated');

figure;
plot(...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.05,3),...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.05,4), 'k.',...
    'markersize', 15)
S = 0:0.01:1; SN = sqrt(S.*(1-S)/10); hold on; plot(S, SN, 'r'); hold off;
title('S-BVA: correlated');

figure;
plot(...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.005,3),...
    appData.BVA.Data.Results(appData.BVA.Data.Results(:,6) < 0.005,4), 'k.',...
    'markersize', 15)
S = 0:0.01:1; SN = sqrt(S.*(1-S)/10); hold on; plot(S, SN, 'r'); hold off;
title('S-BVA: highly correlated');
