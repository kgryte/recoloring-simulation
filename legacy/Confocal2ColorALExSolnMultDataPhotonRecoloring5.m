function [appData, handles, ERRORFLG] = Confocal2ColorALExSolnMultDataPhotonRecoloring5(...
    appData, handles)
%
...
%                             - altered how we perform permutation. Now look for significant neighbors 
%
%   References:
...
%             fprintf('%d Rex photon(s) in wrong bin. Burst No: %d\n', sum(mod(RexIDX,2) == 0), BURST);
    ...
    numWindows = size(recoloredPhotons(:,1),1) - WINDOW + 1;% floor(sum((recoloredPhotons(:,2)==0) + (recoloredPhotons(:,2)==2)) / WINDOW); % number of recolored Gex photons
...
%     pVal = sum(abs(permTest) > abs(rho)) / numPerms; % percentage
    ...
    % [5] Permutation test for significance: (neighbor method)
    ...
    for win = 1 : corrWindows
        ...
        for i = 1 : numPerms
            ...
            % Mix up the indexes:
            indexes = randperm(CORRWINDOW); % num data pts within a correlation window (e.g., 10)
    ...
    minHotSpots = 5; % meaning three regions of high correlation
...
figure; 
plot(...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) > 0.1,3),...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) > 0.1,4), 'k.',...
    'markersize', 15)
...
figure;
plot(...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.05,3),...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.05,4), 'k.',...
    'markersize', 15)
S = 0:0.01:1; SN = sqrt(S.*(1-S)/10); hold on; plot(S, SN, 'r'); hold off;
...
figure;
plot(...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.005,3),...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.005,4), 'k.',...
    'markersize', 15)
S = 0:0.01:1; SN = sqrt(S.*(1-S)/10); hold on; plot(S, SN, 'r'); hold off;
...
plot(...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) > 0.1,3),...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) > 0.1,4), 'b.',...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.05,3),...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.05,4), 'k.',...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.005,3),...
    appData.BCA.Data.Results(appData.BCA.Data.Results(:,9) < 0.005,4), 'r.',...
    'markersize', 15)
S = 0:0.01:1; SN = sqrt(S.*(1-S)/10); hold on; plot(S, SN, 'r'); hold off;
title('S-BCA: Species');
