...
%             fprintf('%d Rex photon(s) in wrong bin. Burst No: %d\n', sum(mod(RexIDX,2) == 0), BURST);
    ...
    numWindows = floor(sum((recoloredPhotons(:,2)==0) + (recoloredPhotons(:,2)==2)) / WINDOW); % number of recolored Gex photons
    ...
    for win = 1 : numWindows
        ...
        recoloredPhotons(1:IDs(end),:) = []; % makes the array smaller at each iteration, thus allowing us to index from 1 as done above.
...
figure; 
plot(appData.BCA.Data.Results(:,5), appData.BCA.Data.Results(:,6), 'b*')
